...
$url = base_url("index.php?component=clarity");
...
if($act=='add' AND (trim($_POST['name']) <> ''))
{
	...
	$query_cekno = "select * from ref_dm_clarity where clarity_name='$nop'";
...
if (isset($error)) {
	...
	if($act =='add') {
            ...
            $query = "INSERT INTO ref_dm_clarity (clarity_name,clarity_description)"
                    . " VALUES ('$_POST[name]','$_POST[description]');";
            //'$_POST[]'
           print_r($query);
        }else if($act=='edit'){
            $query = "UPDATE ref_dm_clarity SET clarity_name='$_POST[name]',clarity_description='$_POST[description]' WHERE clarity_code='$_POST[id]'";
		//print_r($query);
		//break;
