...
//echo $nref."<br/>".$nilai."<br/>".$inv."<br/>".$ak."<br/>".$tr.$rul;

mysql_select_db($database_con_gl, $con_gl);
$query_getakhut = "SELECT gl_trans.akun FROM gl_trans WHERE gl_trans.no_ref='$inv' AND gl_trans.transaksi LIKE '%Hutang Pembelian%'";
...
<table width="100%" border="0" cellspacing="0" cellpadding="5" class="datatable">
  ...
  <tr valign="top" bgcolor="#FFFFFF">
	...
	$run_query = mysql_query($query, $con_gl);*/
	$query2 = "INSERT INTO gl_trans (id, no_ref, transaksi, total, pos, akun, x_ref) VALUES (NULL, '$nref', '$tr', '$nilai', 'D', '$row_getakhut[akun]', '$inv')";
	$run_query2 = mysql_query($query2, $con_gl);
  }else if($rul =='Simpan' AND $nref<>'' AND $tr <>'' AND $nilai > 0 AND $ak <>'') { 
    mysql_select_db($database_con_gl, $con_gl);
	$query = "UPDATE gl_trans SET transaksi= '$tr', total= $nilai, pos= 'D', akun= '$row_getakhut[akun]', x_ref='$inv' WHERE id='$_GET[value]'";
	$run_query = mysql_query($query, $con_gl);
  }
  ?>		
			<?php } ?>
			<?php
			mysql_select_db($database_con_gl, $con_gl);
$query_list_tr = "SELECT gl_trans.x_ref, gl_trans.transaksi, gl_trans.total, gl_trans.id, gl_akun.id as akun_id, gl_akun.akun FROM gl_trans, gl_akun WHERE gl_akun.id = gl_trans.akun AND gl_trans.no_ref='$nref' AND gl_trans.pos='D' ORDER BY gl_trans.id desc";
...
$totalRows_list_tr = mysql_num_rows($list_tr);
			...
			<?php do { ?>
		      <tr valign="top" bgcolor="#FFFFFF">
			    ...
			    <td align="center">
			     <label>
        <button value="&nbsp;" onclick="ajaxEditFormPembayaranHutang(<?php echo $row_list_tr['id']; ?>);" type="button" class="editbutton" title="Edit Transaksi">&nbsp;</button></label><label>
...
$totalRows_sumtot = mysql_num_rows($sumtot);
			  ...
			  <tr valign="top" bgcolor="#FFFFFF">
                  ...
                  <?php echo number_format($row_sumtot['sum(gl_trans.total)'],0,',','.').",-"; ?><input name="total" type="hidden" id="total" value="<?php echo $row_sumtot['sum(gl_trans.total)']; ?>" size="10" maxlength="15" />
                  </label></td>
			      <td align="center" >&nbsp;</td>
			      <td align="center" >&nbsp;</td>
	        </tr>
...
<select name="invoice" id="invoice" style="width:400px;" onChange="hitungHutang(this.value);setTransaksi(this.value)">
  <option value="" style="background:#EEEEEE;font-weight:bold;" value="">Pilih Invoice</option>
...
<?php if($row_totalhut['total'] > $row_totalbyr['sum(gl_trans.total)']) { ?>
  <option value="<?php echo $row_invoi['no_ref']?>" <?php if(!$run_query) { if($inv==$row_invoi['no_ref']) { ?>selected="selected"<?php } } ?>><?php echo $row_invoi['no_ref']?> - <?php echo $listbrg;?></option>
<?php } ?>
  <?php
...
<?php } ?>
              ...
              <td align="center" ><div id="divTotal">
                <label>
                  <input name="nilai" type="text" id="nilai" onchange="javascript:if(this.value > 0) { this.form.debet.value='';}else{ alert('Value invalid !!!');this.value='';}" value="<?php if(!$run_query) { echo $nilai;}?>" size="10"/>
                </label>
              </div></td>
              <td align="center" ><div id="divTransaksi">
                <label>
                  <textarea name="transaksi" cols="40" rows="2" id="transaksi"><?php if(!$run_query) { echo $tr;}?></textarea>
                </label>
              </div></td>
              <td align="center" ><label></label>
                  <label>
                  <input name="add" type="button" id="add" value="&nbsp;" onclick="ajaxManagePembayaranHutang('Tambah');" class="checkbutton" title="Tambah Transaksi"/>
                </label></td>
            </tr>
			<?php //if($tnilai == $total) { ?>
			<?php //} ?>
