...
$url = base_url("index.php?component=polish");
...
if($act=='add' AND (trim($_POST['name']) <> ''))
{
	...
	$query_cekno = "select * from ref_dm_polish where polish_code='$nop'";
...
if (isset($error)) {
	...
	if($act =='add') {
            ...
            $query = "INSERT INTO ref_dm_polish (polish_name,polish_alias)"
                    . " VALUES ('$_POST[name]','$_POST[description]');";
            //'$_POST[]'
          // print_r($query);
        }else if($act=='edit'){
            $query = "UPDATE ref_dm_polish SET polish_name='$_POST[name]',polish_alias='$_POST[description]' WHERE polish_code='$_POST[id]'";
		//print_r($query);
		//break;
