...
$query_arsip = "select id, katagori from gl_katagoriarsip order by katagori";
$arsip = mysql_query($query_arsip, $con_gl) or die(mysql_error());
$row_arsip = mysql_fetch_assoc($arsip);
$totalRows_arsip = mysql_num_rows($arsip);

mysql_select_db($database_con_gl, $con_gl);
$query_getdata = "select * from gl_arsip where id='$_GET[id]'";
$query_getdata .=" ORDER BY tipe, arsip";
...
<?php
	$query = "update gl_arsip set katagori='$_POST[katagori]', keterangan='$ket'";
	$f_dest = "../data_arsip/".$f_nam;
	if($f_tmp <>'') {
		copy($f_tmp,$f_dest);
		$query .=",arsip='$f_nam',tipe='$f_tip'";
...
<form action="" method="POST" enctype="multipart/form-data" name="add" id="add" >
  <table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="datatable">
    <tr valign="top">
      <td align="right" bgcolor="#FFFFFF">Katagori Arsip : </td>
      <td bgcolor="#FFFFFF"><select name="katagori" id="katagori" style="width:200px;">
          <?php if($totalRows_arsip > 0) { do { ?>
          <option value="<?php echo $row_arsip['id'];?>" <?php if($row_arsip['id']==$row_edit['katagori']) { ?>selected="selected"<?php } ?>><?php echo $row_arsip['katagori'];?></option>
          <?php }while($row_arsip = mysql_fetch_assoc($arsip)); } ?>
        </select></td>
    </tr>
    <tr valign="top">
