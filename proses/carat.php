...
$url = base_url("index.php?component=carat");
...
/* Validasi */
if (trim($_POST['name']) == '') {
	$error[] = '- CaratbNama harus diisi';
}
if (trim($_POST['purity']) == '') {
	$error[] = '- Purity harus diisi';
}
...
if (isset($error)) {
	...
	if($act =='add') {
            ...
            $query = "INSERT INTO ref_jw_carat (carat_name,carat_purity)"
                    . " VALUES ('$_POST[name]','$_POST[purity]');";
            //'$_POST[]'
           print_r($query);
        }else if($act=='edit'){
            $query = "UPDATE ref_jw_carat SET carat_name='$_POST[name]',carat_purity='$_POST[purity]' WHERE carat_code='$_POST[id]'";
		//print_r($query);
		//break;
