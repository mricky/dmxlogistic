	...
	if(isset($_SESSION['idagent'])){
		...
		foreach($_SESSION['idagent'] as $idk){
			...
			<?php
					...
					echo "<a style='cursor:pointer;' onclick=\"javascript: kirimRequest('components/agent/detailPricelistAgent.php', 'ajax=true&act=hapus&pos=$i', 'ajaxDiv', 'div');\" title='Hapus data'><img src='images/delete.png' /></a>";
					echo "</td>";
				echo "</tr>";
  ...
  <td align="right"  bgcolor="#00CC99" style="background:none;border:none;">
    ...
    <input type="text" name="service" id="service" size="10" placeholder="Layanan" readonly="readonly" style="background-color:#fff;border:1px solid #999; height:16px; " onClick="NewWindow('index.php?component=service&amp;task=list&amp;open=window','name','825','450','yes');return false" /></td>
              <td align="right" bgcolor="#00CC99" style="background:none;border:none;"><input type="hidden" name="idtransport" id="idtransport" size="2" />
                <input type="text" name="transport" id="transport" size="10" placeholder="Armada" readonly="readonly" style="background-color:#fff;border:1px solid #999; height:16px; " onclick="NewWindow('index.php?component=agenthandling&amp;task=listmodatransport&amp;open=window','name','825','450','yes');return false" /></td>
    <td align="right" bgcolor="#00CC99" style="background:none;border:none;"><input type="hidden" name="idorigin" id="idorigin" size="2" />
      <input type="text" name="origin" id="origin" size="10" placeholder="Kota Asal" readonly="readonly" style="background-color:#fff;border:1px solid #999; height:16px; " onclick="NewWindow('index.php?component=kotafrom&amp;task=list&amp;open=window','name','825','450','yes');return false" /></td>
              ...
              <td align="right" bgcolor="#00CC99" style="background:none;border:none;"><input type="hidden" name="idnextdest" id="idnextdest" size="2" />
                  <input type="text" name="nextdest" id="nextdest" size="10" placeholder="Kota Terusan" readonly="readonly" style="background-color:#fff;border:1px solid #999; height:16px; " onclick="NewWindow('index.php?component=kotaterusan&amp;task=list&amp;open=window','name','825','450','yes');return false" /></td>
              ...
              <td align="center" bgcolor="#00CC99" style="background:none;border:none;"><input name="add" type="button" id="add" value="&nbsp;" onclick="var err=''; 
                 ...
                 if(document.getElementById('origin').value=='') err+='origin harus dipilih\n';  				                 if(document.getElementById('destination').value=='') err+='destination harus dipilih\n';                 if(document.getElementById('nextdest').value=='') err+='terusan harus dipilih\n';                 
                   ...
                   if(document.getElementById('minpackage').value=='') err+='rate agent harus dipilih\n';           
					if(err!='') alert(err);
					else kirimRequest('components/agent/detailPricelistAgent.php', 'ajax=true&amp;act=addPricelistAgent&amp;idservice='+document.getElementById('idservice').value+'&amp;service='+document.getElementById('service').value+'&amp;idtransport='+document.getElementById('idtransport').value+'&amp;transport='+document.getElementById('transport').value+'&amp;idorigin='+document.getElementById('idorigin').value+'&amp;origin='+document.getElementById('origin').value+'&amp;iddestination='+document.getElementById('iddestination').value+'&amp;destination='+document.getElementById('destination').value+'&idnextdest='+document.getElementById('idnextdest').value+'&nextdest='+document.getElementById('nextdest').value+'&amp;rate='+document.getElementById('rate').value+'&amp;minpackage='+document.getElementById('minpackage').value, 'ajaxDiv', 'div');" class="checkbutton" title="Add Transaksi"/></td>
  </tr>
</table>
