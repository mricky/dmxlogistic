...
$query_def_ak = "SELECT gl_company.a_hut FROM gl_company";
...
mysql_select_db($database_con_gl, $con_gl);
$query_klas = "SELECT kd, klasifikasi, keterangan, neraca, ruglab, tipe FROM gl_klas where tipe = 6 ORDER BY kd,klasifikasi ASC";
...
?>
          ...
          <?php do { ?>
          <option value="<?php echo $row_akun['id']?>" <?php if($row_def_ak['a_hut']==$row_akun['id']) { ?>selected="selected"<?php } ?>>-<?php echo $row_akun['akun']?></option>
...
<?php }else{ ?>
		...
		<?php } ?>
		<label>
        <input name="tipetrans" type="radio" value="Tunai" <?php if($_GET[tipe]=='Tunai') { ?>checked="checked"<?php } ?> onchange="if(this.checked) { this.form.jatuhtempo.value='';this.form.jatuhtempo.disabled='disabled';cektipetransjual(this.value);}"/>
        Tunai</label>
        <label>
