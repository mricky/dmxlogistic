...
function getBiayaOprs(){
	...
	ajaxRequest.onreadystatechange = function(){
		if(ajaxRequest.readyState == 4){
			var ajaxDisplay = document.getElementById('biaya');
			ajaxDisplay.value = ajaxRequest.responseText;
			var ajaxDisplay2 = document.getElementById('xbiaya');
			ajaxDisplay2.value = ajaxRequest.responseText;
			/*var ajaxDisplay3 = document.getElementById('totalafterdiskon');
			ajaxDisplay3.value = ajaxRequest.responseText;*/
			document.getElementById('totalafterdiskon').value=document.getElementById('xbiaya').value*document.getElementById('diskontotal').value;
			document.getElementById('bobot').value='10';
			var ppn=0.1*parseInt(document.getElementById('xbiaya').value);
			var totalakhir=parseInt(document.getElementById('xbiaya').value)-ppn;
			document.getElementById('pajak').value=ppn;
			document.getElementById('ntotal').value=totalakhir;
		}
		if (ajaxRequest.readyState == 3) {
			var ajaxDisplay = document.getElementById('biaya');
			ajaxDisplay.value = "<img src='images/ajax-loader.gif' />";
		}
	}
	var bar = document.getElementById('barang').value;
	var ken = document.getElementById('kendaraan').value;
	var queryString = "?kid=" + ken + "&barang=" + bar;
	ajaxRequest.open("GET", "include/get_biayaoprs.php" + queryString, true);
	ajaxRequest.send(null);
}
// ---
...
function ajaxSetKendaraan(act,rid) {
	...
	var queryString = "?action=" + act + "&rid=" + rid + "&kontrak=" + kon + "&kendaraan=" + ken + "&harga=" + har + "&diskon=" + dis + "&total=" + tot + "&transaksi=" + tra;
