...
$url = base_url("index.php?component=symmetry");
...
if($act=='add' AND (trim($_POST['name']) <> ''))
{
	...
	$query_cekno = "select * from ref_dm_symmetry where symmetry_name='$nop'";
...
if (isset($error)) {
	...
	if($act =='add') {
            ...
            $query = "INSERT INTO ref_dm_symmetry (symmetry_name,symmetry_alias)"
                    . " VALUES ('$_POST[name]','$_POST[description]');";
            //'$_POST[]'
          // print_r($query);
        }else if($act=='edit'){
            $query = "UPDATE ref_dm_symmetry SET symmetry_name='$_POST[name]',symmetry_alias='$_POST[description]' WHERE symmetry_code='$_POST[id]'";
		//print_r($query);
		//break;
