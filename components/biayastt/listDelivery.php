...
$query_klas = "select NAMAAGENT,NAMALAYANAN,NAMAJNSKIRIM,KOTAASAL,KOTATUJUAN,RATEHANDLING,MINPAKET from v_pricelistagent WHERE IDAGENTHANDLING = 2 GROUP BY  HANDLING,NAMAAGENT";
...
<h1>Pilih Pricelist Delivery Agent</h1>
<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="datatable">
  <tr valign="top" bgcolor="#FFFFFF" class="hide">
	...
	$query = mysql_query("select NAMAAGENT,NAMALAYANAN,NAMAJNSKIRIM,KOTAASAL,KOTATUJUAN,RATEHANDLING,MINPAKET from v_pricelistagent WHERE IDAGENTHANDLING = 2 AND NAMAAGENT like '%$key%' GROUP BY HANDLING,NAMAAGENT") or die (mysql_error());
	while(list($agent, $layanan, $armada,$asal,$tujuan,$rate,$minpackage) = mysql_fetch_row($query)){
		echo "<tr>";
		echo "<td>$agent</td><td>$layanan</td><td>$armada</td><td>$asal</td><td>$tujuan</td><td>$rate</td><td>$minpackage</td><td align='center'><input type='button' value='pilih' onclick=\"javascript: window.opener.document.getElementById('costdelivery').value='$rate'; window.opener.document.getElementById('agentdelivery').value='$agent'; window.close(1000);\" /></td>";
		echo "</tr>";
	}
	?>
 </table>
