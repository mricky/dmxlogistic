...
$query_klas = "SELECT * from layanan";
...
<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="datatable">
  <tr valign="top" bgcolor="#FFFFFF" class="hide">
        <td colspan="11" align="right" style="border-bottom:solid 1px #E2EAC1;border-right:solid 1px #E2EAC1;">
			...
			<!-- Edit by suwondo -->
			  Enter a service name to filter: 
				<label>
					<input name="cr_txt" type="text" id="cr_txt" placeholder="Find by: Layanan" onclick="this.select(); $(this).autocomplete('components/service/search.autocomplete.php', { width: 330 });" size="45" maxlength="100" onkeyup="javascript: $(this).autocomplete('components/service/search.autocomplete.php', { width: 330 }); $(this).result(function(event, data, formatted){ idx = formatted.split(' - '); window.location.href='index.php?component=service&task=list&open=window&cr_kel=ALL&cr_txt='+idx[0]; });" <?php echo $_REQUEST['cr_txt'];?> />
				</label>
	...
	$query = mysql_query("select IDLAYANAN,KODELAYANAN,NAMALAYANAN from layanan where NAMALAYANAN like '%$key%'") or die (mysql_error());
	while(list($id, $kodelayanan, $namalayanan) = mysql_fetch_row($query)){
		echo "<tr>";
		echo "<td>$kodelayanan</td><td>$namalayanan</td><td align='center'><input type='button' value='pilih' onclick=\"javascript: window.opener.document.getElementById('idservice').value='$id'; window.opener.document.getElementById('service').value='$namalayanan'; window.close(1000);\" /></td>";
		echo "</tr>";
	}
	?>
 </table>