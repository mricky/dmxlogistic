...
<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="datatable">
	...
	<tr>
		<th width="266"><strong>Nama Karyawan</strong></th>
	...
	$query = mysql_query("SELECT * FROM gl_kontak WHERE tkaryawan<>'' AND (nama like '%$key%' OR alamat like '%$key%' OR tlp like '%$key%') order by id asc") or die (mysql_error());
	while(list($id, $nama, $alamat, $tlp) = mysql_fetch_row($query)){
		echo "<tr>";
		echo "<td>$nama</td><td>$alamat</td><td>$tlp</td><td align='center'><input type='button' value='pilih' onclick=\"javascript: window.opener.document.getElementById('namacustomer').value='$id'; window.opener.document.getElementById('txtnamacustomer').value='$namacustomer'; window.close(1000);\" /></td>";
		echo "</tr>";	}
	?>
 </table>
