...
$query_edit = sprintf("SELECT * FROM agent inner  join agenthanding b ON (agent.IDAGENTHANDLING = b.ID) WHERE ID_AGENT = '$_GET[id]'", $colname_edit);
$agentid = $_GET['id'];
...
<h1>Pricelist Agent</h1>
<form action="proses/pricelistAgent.php?act=editPriceAgent" method="post" name="editPriceAgent" id="editPriceAgent">

  <table width="95%" border="0" align="center" cellpadding="5" cellspacing="0" class="datatable">

    <tr valign="top">
      <td align="right">Handling :</td>
      <td><input name="idhandling" type="hidden" id="idhandling"  size="30" maxlength="100" value="<?php echo $row_edit['IDAGENTHANDLING']; ?>"/>
        <input name="handling" type="text" id="handling" size="30" value="<?php echo $row_edit['NAMA']; ?>" readonly="readonly" maxlength="100"  /></td>
    </tr>
    <tr valign="top">
      <td width="21%" align="right">Nama * :</td>
      <td width="72%"><label>
      <input name="nama" type="text" id="nama" size="30" maxlength="100"  value="<?php echo $row_edit['NAMAAGENT']; ?>"/>
      <input name="id" type="hidden" id="id" size="30" maxlength="100"  value="<?php echo $row_edit['ID_AGENT']; ?>"/>
      </label></td>
    </tr>
    ...
    <tr>
      ...
      <td valign="top"><input name="npwp" type="text" id="npwp" size="30" maxlength="100" value="<?php echo $row_edit['NPWP']; ?>"/></td>
    </tr>
    <tr>
      <td align="right" valign="top">REKENING BANK :</td>
      <td valign="top"><input name="rekening" type="text" id="rekening" size="30" maxlength="100" value="<?php echo $row_edit['ACCOUNTNUMBER']; ?>" /></td>
    </tr>
    <tr>
      <td align="right" valign="top">ATAS NAMA :</td>
      <td valign="top"><input name="atas_nama" type="text" id="atas_nama" size="30" maxlength="100" value="<?php echo $row_edit['ATASNAMA']; ?>" /></td>
    </tr>
    <tr>
    ...
    <tr>
      <td colspan="2" align="right" valign="top"><?php
			...
			mysql_select_db($database_con_gl, $con_gl);
			$query = mysql_query("select IDAGENT,IDLAYANAN,NAMALAYANAN,IDJNSKIRIM,NAMAJNSKIRIM,IDORIGIN, KOTAASAL,IDDEST,KOTATUJUAN,IDTERUSAN,NAMATERUSAN,RATEHANDLING,MINPAKET from v_pricelistagent where IDAGENT = '$row_edit[ID_AGENT]'") or die (mysql_error());
			$i=0;
			
			
			
			while(list($IDAGENT,$IDLAYANAN,$NAMALAYANAN,$IDJNSKIRIM,$NAMAJNSKIRIM,$IDORIGIN, $KOTAASAL,$IDDEST,$KOTATUJUAN,$IDTERUSAN,$NAMATERUSAN,$RATEHANDLING,$MINPAKET) = mysql_fetch_row($query)){
			
				//$_SESSION['tglrealisasi'][$i] 			= $tglrealisasi;
				
			
				$_SESSION['idagent'][$i] = $IDAGENT;
				$_SESSION['agent'][$i] = '';				
				$_SESSION['idservice'][$i] = $IDLAYANAN;
				$_SESSION['service'][$i] = $NAMALAYANAN;
				$_SESSION['idorigin'][$i] = $IDORIGIN;
				$_SESSION['origin'][$i] = $KOTAASAL;				
				$_SESSION['iddestination'][$i] = $IDDEST;				
				...
				$_SESSION['idmodatransport'][$i] = $IDJNSKIRIM;				
				$_SESSION['rate'][$i] = $RATEHANDLING;																	
				$_SESSION['minpackage'][$i] = $MINPAKET;																																	           		
