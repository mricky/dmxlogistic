...
$url = base_url("index.php?component=jwcategory");
//$nop = str_replace(" ","",strtoupper($_POST['merk']));
$nop = strtoupper($_POST['categoryName']);
$ope = $_POST['open'];
...
if (trim($_POST['categoryName']) == '') {
	$error[] = '- Nama Category harus diisi';
}
if (trim($_POST['symbol']) == '') {
	$error[] = '- Symbol harus diisi';
}
...
if($act=='add' AND (trim($_POST['barcode']) <> ''))
{
	...
	$query_cekno = "select category_name from ref_jw_category where category_name='$nop'";
	$cekno       = mysql_query($query_cekno, $con_gl);
	$row_cekno   = mysql_fetch_assoc($cekno);
	$total_cekno = mysql_num_rows($cekno);
	if($total_cekno > 0) { 
		$error[] = '- Category Name '.$nop.' sudah terdaftar';
	}
}
...
if (isset($error)) {
	...
	if($act =='add') {
            ...
            $query = "INSERT INTO ref_jw_category (category_name,symbol)"
                    . " VALUES ('$_POST[categoryName]','$_POST[symbol]');";
            //'$_POST[]'
           //print_r($query);
        }else if($act=='edit'){
            $query = "UPDATE ref_jw_category SET category_name='$_POST[categoryName]',symbol='$_POST[symbol]' WHERE category_code='$_POST[id]'";
		//print_r($query);
		//break;
