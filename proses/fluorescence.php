...
$url = base_url("index.php?component=fluorescence");
...
if($act=='add' AND (trim($_POST['name']) <> ''))
{
	...
	$query_cekno = "select * from ref_dm_fluorescence where fluorescence_name='$nop'";
	$cekno       = mysql_query($query_cekno, $con_gl);
	$row_cekno   = mysql_fetch_assoc($cekno);
	$total_cekno = mysql_num_rows($cekno);
	if($total_cekno > 0) { 
		$error[] = '- Clarity Name '.$nop.' sudah terdaftar';
	}
}
...
if (isset($error)) {
	...
	if($act =='add') {
            ...
            $query = "INSERT INTO ref_dm_fluorescence (fluorescence_name,fluorescence_description)"
                    . " VALUES ('$_POST[name]','$_POST[description]');";
            //'$_POST[]'
           print_r($query);
        }else if($act=='edit'){
            $query = "UPDATE ref_dm_fluorescence SET fluorescence_name='$_POST[name]',fluorescence_description='$_POST[description]' WHERE fluorescence_code='$_POST[id]'";
		//print_r($query);
		//break;
