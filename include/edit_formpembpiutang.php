...
$query_listbayar = "SELECT gl_trans.transaksi, gl_trans.total, gl_trans.x_ref, gl_trans.id FROM gl_trans WHERE gl_trans.no_ref='$_GET[noreferensi]' AND gl_trans.pos='K'";
  ...
  <?php if($_GET[refid] <> $row_listbayar['id']) { ?>
    <tr valign="top" bgcolor="#FFFFFF">
      ...
      <td width="14%" align="center">
			     <label>
        <button value="<?php echo $row_list_tr['id']; ?>" onclick="ajaxEditFormPembayaranPiutang(this.value);" type="button" class="editbutton" title="Edit Transaksi">&nbsp;</button></label><label>
...
?>
        <option value="<?php echo $row_invoi['no_ref']?>"<?php if (!(strcmp($row_invoi['no_ref'], $row_getxref['x_ref']))) {echo "selected=\"selected\"";} ?>><?php echo $row_invoi['no_ref'];?> - <?php echo $listbrg;?></option>
        <?php
...
?>
      ...
      <td align="center">        <label>
        <input name="add" type="button" id="add" value="&nbsp;" onclick="ajaxManagePembayaranPiutang('Simpan',<?php echo $row_listbayar['id']; ?>);" class="checkbutton" title="Simpan Transaksi"/>
        </label>      </td>
    </tr>
	<?php } ?>
