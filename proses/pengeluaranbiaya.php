...
<?php
$act = $_GET[act];
$url = base_url("index.php?component=tagihan");
...
if (isset($error)) {
	...
	$now = strtotime('now');
	$query = "INSERT INTO tagihan (IDAGENT,TGLDATANGINV,TGLINV,NOINV,JUMLAHTAGIHAN,KETERANGAN,CREATED_AT,CREATED_BY) VALUES ('$_POST[idagent]','$_POST[tanggaldatanginvoice]','$_POST[tanggalinvoice]','$_POST[noinvoice]','$_POST[nominal]','$_POST[keterangan]','$_SESSION[nama]','$now')";
	...
	$query ="UPDATE gl_pengeluaranuang SET tgl='$_POST[tgl]',id_jenis='$_POST[jenispengeluaran]',notransaksi='$_POST[notransaksi]',nobukti='$_POST[nobukti]',nominal='$_POST[nominal]',keterangan='$_POST[keterangan]',updated_by = '$_SESSION[nama]', updated_at = '$today' WHERE id='$_POST[idpengeluaran]'";
		 ...
		 $now = strtotime('now');
	     $NOTRANSAKSI = "CO-".strtotime('now');
	  // cash out
	  $query = "INSERT INTO transaksipembayarantagihan (IDTAGIHAN,NOTRANSAKSI,NOTRANSFER,TGLBAYAR,IDMETODEBAYAR,IDJENISBAYAR,TOTALBAYAR,KETERANGAN,CREATED_AT,CREATED_BY) VALUES ($_GET[IDINVOICE],'$NOTRANSAKSI','$_POST[notransfer]','$_POST[tanggalbayar]','$_POST[cr_tipebayar]','$_POST[cr_jenisbayar]','$_POST[nominalbayar]','$_POST[keterangan]','$_SESSION[nama]','$now')";
