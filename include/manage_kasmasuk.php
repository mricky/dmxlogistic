...
<table width="100%" border="0" cellspacing="0" cellpadding="5" class="datatable">
	...
	$run_query = mysql_query($query, $con_gl);
            ...
            <tr>
              ...
              <th width="10%" align="center"><strong>Nilai (K) </strong></th>
  ...
  <tr valign="top" bgcolors="#FEAA8D">
	...
	mysql_select_db($database_con_gl, $con_gl);
	$query = "INSERT INTO gl_trans (id, no_ref, transaksi, total, pos, akun) VALUES (NULL, '$nref', '$tr', '$nilai', 'K', '$ak')";
	$run_query = mysql_query($query, $con_gl);
  }else if($rul =='Simpan' AND $nref<>'' AND $tr <>'' AND $nilai > 0 AND $ak <>'') { 
    mysql_select_db($database_con_gl, $con_gl);
	$query = "UPDATE gl_trans SET transaksi= '$tr', total= $nilai, pos= 'K', akun= '$ak' WHERE id='$_GET[value]'";
	$run_query = mysql_query($query, $con_gl);
  }
  ?>		
			<?php } ?>
			<?php
			mysql_select_db($database_con_gl, $con_gl);
$query_list_tr = "SELECT gl_trans.transaksi, gl_trans.total, gl_trans.id, gl_akun.id as akun_id, gl_akun.akun FROM gl_trans, gl_akun WHERE gl_akun.id = gl_trans.akun AND gl_trans.no_ref='$nref' AND gl_trans.pos='K' ORDER BY gl_trans.id desc";
...
$totalRows_list_tr = mysql_num_rows($list_tr);
			...
			<?php $tottrans = 0; do { ?>
		      <tr valign="top" bgcolor="#FFFFFF">
			    ...
			    <td width="35%" align="center"><?php echo $row_list_tr['transaksi']; ?></td>
			    <td width="10%" align="center">
			      <label>
        <button value="<?php echo $row_list_tr['id']; ?>" onclick="ajaxEditFormKasmasuk(<?php echo $row_list_tr['id']; ?>);" type="button" class="editbutton" title="Edit Transaksi">&nbsp;</button>
      </label>
			      <label>
                  <input name="hapus" type="button" id="hapus" value="&nbsp;" onclick="ajaxManageKasmasuk('Hapus',<?php echo $row_list_tr['id']; ?>);" title="Hapus Transaksi" class="deletebutton"/>
                </label></td>
		    </tr>
...
$totalRows_sumtot = mysql_num_rows($sumtot);
            ...
            <tr valign="top" bgcolor="#FFFFFF">
              <td width="10%" align="center"><div id="kodeDiv">
                  <label>
                  <input name="akunkd" type="text" id="akunkd" onchange="cekAkun(this.value);" value="<?php if(!$run_query) { echo $ak; }?>" size="10" maxlength="10"/>
                  </label>
              </div></td>
...
?>
              ...
              <td width="10%" align="center"><label>
                <input name="nilai" type="text" id="nilai" size="10" value="<?php if(!$run_query) { echo $nilai; }?>"/>
              </label></td>
              <td width="35%" align="center"><label>
                <textarea name="transaksi" cols="45" rows="3" id="transaksi"><?php if(!$run_query) { echo $tr; }?></textarea>
              </label></td>
              <td width="10%" align="center"><label>
                <input name="add" type="button" id="add" value="&nbsp;" onclick="ajaxManageKasmasuk('Tambah');" class="checkbutton" title="Tambah Transaksi"/>
              </label></td>
            </tr>
			<?php //if($tnilai == $total) { ?>
			<tr>
