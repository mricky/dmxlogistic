...
$act = $_GET[act];
$url = "index.php?component=pembelian";
...
if (isset($error)) {
	...
	$query = "INSERT INTO gl_rtrans (no_ref, tgl, keterangan, gudang, kontak, jenis, jatuhtempo, recipient, akbiaya, akpjk) VALUES ('$_POST[noreferensi]', '$_POST[tanggal]', '$_POST[keterangan]', '$_POST[gudang]', '$_POST[dari]', '$_POST[jenis]', '$_POST[jatuhtempo]', '$_POST[recipient]', '$_POST[akunbiaya]', '$_POST[akunpajak]')";
		...
		if($_POST[akunkredit]<>'' AND $_POST[saldohutang] > 0) {
  			...
  			mysql_select_db($database_con_gl, $con_gl);
 			mysql_query("INSERT INTO gl_trans (id, no_ref, transaksi, total, pos, akun) VALUES (NULL, '$_POST[noreferensi]', 'Hutang Pembelian ke $row_getkpd[nama]', '$nil', 'K', '$_POST[akunkredit]')", $con_gl);
  		}
	...
	$row_getkpd = mysql_fetch_assoc($getkpd);
		if($_POST[akunkredit]<>'' AND $_POST[saldohutang] > 0) {
  		...
  		if($_POST[akunpajak] <>'' AND $_POST[pajak] > 0) {
  			mysql_select_db($database_con_gl, $con_gl);
  			if($_POST[idpajak]=='') {
  				$wew = "INSERT INTO gl_trans (id, no_ref, transaksi, total, pos, akun) VALUES (NULL, '$_POST[noreferensi]', 'Beban Pajak', '$_POST[pajak]', 'D', '$_POST[akunpajak]')";
  		...
  		if($_POST[bayar] <> 0) {
			...
			if($_POST[idbayar] <>'') {
				mysql_query("delete from gl_trans where id='$_POST[bayar]'", $con_gl);
			}
		}
	}else{
