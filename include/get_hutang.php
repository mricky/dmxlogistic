...
$query_gethutang = "SELECT sum(gl_trans.total) FROM gl_trans WHERE gl_trans.no_ref='$_GET[invoice]' AND gl_trans.pos='K' AND gl_trans.transaksi LIKE '%%Hutang%%'";
...
mysql_select_db($database_con_gl, $con_gl);
$query_getretur = "select sum(gl_trans.total), gl_rtrans.no_ref from gl_rtrans, gl_trans where gl_rtrans.no_ref LIKE '%%PR%%' AND gl_rtrans.x_ref ='$_GET[invoice]' AND gl_rtrans.no_ref = gl_trans.no_ref AND gl_trans.pos='K' group by gl_rtrans.no_ref";
...
mysql_select_db($database_con_gl, $con_gl);
$query_bayar = "SELECT sum(gl_trans.total) FROM gl_trans WHERE gl_trans.x_ref='$_GET[invoice]' AND gl_trans.pos='D'";
